...
const Profile = () => {
  ...
  const saveChanges = async () => {
    if (validateProfile()) {
      try {
        const response = await apiClient.post(UPDATE_PROFILE_ROUTE, { firstName, lastName }, { withCredentials: true });
    ...
    <div className="bg-slate-900 h-[100vh] flex items-center justify-center flex-col gap-10">
      <div className="flex flex-col gap-10 w-[80vw]">
        <div onClick={handleNavigate}>
          <IoArrowBack className="text-4xl text-white text-opacity-90 cursor-pointer" />
        </div>
        <div className="grid grid-cols-2">
          <div className="h-full w-32 relative flex items-center justify-center" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            <Avatar className="h-32 w-32 rounded-full overflow-hidden">
            {
              ...
              :
              <div className="uppercase h-32 w-32 text-5xl border-[1px] flex items-center justify-center rounded-full bg-[#712c4a57] text-[#ff006e] border-[#ff006faa]">
            ...
            <input type="file" name="profile-image" ref={fileInputRef} className="hidden" onChange={handleImageChange} accept=".png, .jpg, .jpeg, .svg, .webp" />
          </div>
